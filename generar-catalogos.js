...
// Extrae la etiqueta corta de la variante (ej: "50cm") a partir del display_name
// que arma Odoo, del tipo "Cadena Plata Turbillon Laser 050 (50cm 11.8g)".
// Se queda con el primer "token" del contenido entre paréntesis para que quepa
// en el catálogo (ej: "50cm 11.8g" → "50cm").
function extraerMedidaDeVariante(displayName) {
  if (!displayName) return null;
  const m = displayName.match(/\(([^)]+)\)\s*$/);
  if (!m) return null;
  const valorCompleto = m[1].trim();
  const primerAtributo = valorCompleto.split(',')[0].trim();
  const token = primerAtributo.split(/\s+/)[0];
  return token || primerAtributo || null;
}
...
// Solo stock y precio — rápido. Devuelve la lista a nivel de VARIANTE (una
// fila por cada medida/talla), tal como antes — esto se sigue usando tal
// cual para el HTML de consulta de stock. El agrupado por producto para el
// catálogo PDF se hace aparte, en agruparPorMedida().
async function fetchProductos() {
  console.log('📦 Obteniendo productos de Odoo...');
  const [rawVariantes, rawTemplates] = await Promise.all([
    odooCall('product.product', 'search_read', [
      [], ['default_code','name','display_name','list_price','qty_available','virtual_available','categ_id','product_tmpl_id']
    ]),
    odooCall('product.template', 'search_read', [
      [], ['master_code','product_variant_count']
    ]),
  ]);
  ...
  for (const t of rawTemplates) {
    tmplInfo[t.id] = {
      masterCode:   (t.master_code || '').toString().trim(),
      variantCount: t.product_variant_count || 1,
    };
  }

  const productos = rawVariantes
    .map(p => ({
  ...
  return { productos, tmplInfo };
}

// Agrupa las variantes de un mismo producto (ej: Cadena Turbillon 050 en
// medidas 50/55/60cm) en UNA sola entrada de catálogo, con las medidas
// disponibles listadas debajo — igual que se hace en el chat de WhatsApp.
// Se aplica sobre una lista YA filtrada por familia/categoría (y por
// `filtro`, si el catálogo tiene uno), para que el filtro siga operando
// sobre los códigos de variante tal como antes.
function agruparPorMedida(prods, tmplInfo) {
  const porTmpl = {};
  for (const v of prods) {
    const tid = v.TmplId;
    if (!porTmpl[tid]) porTmpl[tid] = [];
    porTmpl[tid].push(v);
  }

  const resultado = [];
  for (const tid of Object.keys(porTmpl)) {
    const grupo = porTmpl[tid];
    const info  = tmplInfo[tid] || {};
    const variantCount = info.variantCount || grupo.length;

    if (variantCount <= 1 || grupo.length <= 1) {
      // Producto simple, sin variantes reales — comportamiento de siempre.
      resultado.push({ ...grupo[0], ImageCode: grupo[0].Default_code });
      continue;
    }

    // Producto con variantes reales (ej: distintas medidas): UNA sola
    // entrada de catálogo, con las medidas disponibles listadas debajo.
    const primero = grupo[0];
    const medidas = grupo
      .map(v => ({
        label:        extraerMedidaDeVariante(v.DisplayName) || limpiarCodigo(v.Default_code),
        codigo:       v.Default_code,
        soloLlegando: v.Stock === 0 && v.Incoming > 0,
      }))
      .sort((a, b) => a.label.localeCompare(b.label, 'es', { numeric: true }));

    const masterCode = info.masterCode || limpiarCodigo(primero.Default_code);
    // Para la imagen: usar la variante con más stock (más probable que
    // tenga una foto representativa cargada).
    const conImagen = [...grupo].sort((a, b) => b.Stock - a.Stock)[0];

    resultado.push({
      Default_code: masterCode,
      Name:         primero.Name,
      Price:        primero.Price,
      Stock:        grupo.reduce((s, v) => s + v.Stock, 0),
      Incoming:     grupo.reduce((s, v) => s + v.Incoming, 0),
      Category:     primero.Category,
      TmplId:       primero.TmplId,
      ImageCode:    conImagen.Default_code,
      Medidas:      medidas,
    });
  }
  return resultado;
...
async function generarPDF(nombreArchivo, productos, orden, caracteristicas, imgs) {
  ...
  for (const p of productos) {
    ...
    const cached = imgs[p.ImageCode || p.Default_code];
    ...
    if (p.Medidas && p.Medidas.length) {
      // Producto con varias medidas (ej: 50cm / 55cm / 60cm) — se listan
      // todas debajo, en vez de repetir el mismo producto una vez por medida.
      const txt = p.Medidas
        .map(m => m.label + (m.soloLlegando ? '*' : ''))
        .join('  ·  ');
      doc.fontSize(7).fillColor('#555555').font('Helvetica')
        .text(txt, x+1*MM, infoY+7*MM,
          { width: cellW-2*MM, align: 'center' });
    } else {
...
async function main() {
  ...
  const [{ productos: todos, tmplInfo }, caracteristicas] = await Promise.all([
    fetchProductos(),
  ...
  for (const cat of catalogos) {
    ...
    prods = agruparPorMedida(prods, tmplInfo);
    prods.forEach(p => { const c = p.ImageCode || p.Default_code; if (c) codigosNecesarios.add(c); });
  }
